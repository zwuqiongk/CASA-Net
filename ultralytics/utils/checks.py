...
def check_amp(model):
    ...
    try:
        ...
        # Try local weights first to avoid download
        local_weights = [
            Path(__file__).parent.parent.parent / "weights" / "yolo26n.pt",
            Path(__file__).parent.parent.parent.parent / "weights" / "yolo26n.pt",
            Path("weights/yolo26n.pt"),
        ]
        yolo_path = None
        for p in local_weights:
            if p.exists():
                yolo_path = str(p)
                break
        ...
        assert amp_allclose(YOLO(yolo_path), im)
