...
class TaskAlignedAssigner(nn.Module):
    ...
    def __init__(
        ...
        topk2=None,
        estal: bool = False,
        estal_min_assign: int = 6,
        estal_spatial_tolerance: float = 0.7,
        estal_threshold_scale: float = 0.20,
        estal_base_threshold: float = 0.3,
        estal_small_threshold: int = 8,
    ):
        ...
        Args:
            ...
            estal (bool, optional): Enable E-STAL (Enhanced Small-Target-Aware Label Assignment).
            estal_min_assign (int, optional): E-STAL minimum positive assignments for small objects.
            estal_spatial_tolerance (float, optional): E-STAL spatial tolerance for tiny objects.
            estal_threshold_scale (float, optional): E-STAL adaptive threshold scaling factor.
            estal_base_threshold (float, optional): E-STAL base IoU threshold.
            estal_small_threshold (int, optional): E-STAL pixel threshold for small object definition.
        """
        super().__init__()
        self.topk = topk
        ...
        self.eps = eps
        self.estal = estal
        self.estal_min_assign = estal_min_assign
        self.estal_spatial_tolerance = estal_spatial_tolerance
        self.estal_threshold_scale = estal_threshold_scale
        self.estal_base_threshold = estal_base_threshold
        self.estal_small_threshold = estal_small_threshold
    ...
    def get_pos_mask(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, anc_points, mask_gt):
        ...
        if self.estal:
            mask_pos = self._estal_enhance(mask_pos, overlaps, gt_bboxes, mask_gt, anc_points, align_metric)
    ...
    def _estal_enhance(self, mask_pos, overlaps, gt_bboxes, mask_gt, anc_points, align_metric):
        """E-STAL: Enhanced Small-Target-Aware Label Assignment.

        Two key enhancements over vanilla TaskAlignedAssigner:
        1. Adaptive IoU threshold: lowers threshold for small objects based on scale factor.
        2. Minimum assignment guarantee: ensures tiny objects get at least min_assign positives.

        Args:
            mask_pos: Current positive mask [bs, n_max_boxes, n_anchors].
            overlaps: IoU values [bs, n_max_boxes, n_anchors].
            gt_bboxes: Ground truth boxes [bs, n_max_boxes, 4] in xywh format.
            mask_gt: Valid GT mask [bs, n_max_boxes, 1].
            anc_points: Anchor center points [n_anchors, 2].
            align_metric: Alignment metric [bs, n_max_boxes, n_anchors].

        Returns:
            Enhanced mask_pos with E-STAL adjustments.
        """
        bs, n_max_boxes, n_anchors = mask_pos.shape
        device = mask_pos.device

        w_gt = gt_bboxes[..., 2]
        h_gt = gt_bboxes[..., 3]
        min_side = torch.minimum(w_gt, h_gt)

        scale_factor = (min_side / 32.0).clamp(min=0.1, max=1.0)
        adaptive_thresholds = (self.estal_base_threshold - scale_factor * self.estal_threshold_scale).clamp(
            min=0.15, max=0.5
        )

        for b in range(bs):
            for gt_idx in range(n_max_boxes):
                if not mask_gt[b, gt_idx].any():
                    continue

                pos_indices = mask_pos[b, gt_idx].nonzero(as_tuple=True)[0]
                if pos_indices.numel() == 0:
                    continue

                gt_overlaps = overlaps[b, gt_idx, pos_indices]

                threshold = adaptive_thresholds[b, gt_idx]
                high_iou_mask = gt_overlaps >= threshold

                if high_iou_mask.any():
                    keep = pos_indices[high_iou_mask]
                    drop = pos_indices[~high_iou_mask]
                    mask_pos[b, gt_idx, drop] = 0
                else:
                    sorted_idx = gt_overlaps.argsort(descending=True)
                    keep_count = max(1, min(self.estal_min_assign // 2, pos_indices.numel()))
                    keep = pos_indices[sorted_idx[:keep_count]]
                    drop = pos_indices[sorted_idx[keep_count:]]
                    mask_pos[b, gt_idx, drop] = 0

                current_pos_count = mask_pos[b, gt_idx].sum().item()
                if min_side[b, gt_idx] < self.estal_small_threshold and current_pos_count < self.estal_min_assign:
                    gt_center_x = gt_bboxes[b, gt_idx, 0]
                    gt_center_y = gt_bboxes[b, gt_idx, 1]
                    distances = ((anc_points[:, 0] - gt_center_x) ** 2 +
                                 (anc_points[:, 1] - gt_center_y) ** 2).sqrt()

                    spatial_radius = min_side[b, gt_idx] * self.estal_spatial_tolerance
                    within_radius = distances <= spatial_radius

                    candidates = within_radius & (~mask_pos[b, gt_idx].bool())
                    candidate_indices = candidates.nonzero(as_tuple=True)[0]

                    if candidate_indices.numel() > 0:
                        candidate_metrics = align_metric[b, gt_idx, candidate_indices]
                        needed = self.estal_min_assign - int(current_pos_count)
                        n_add = min(needed, candidate_indices.numel())
                        top_indices = candidate_metrics.topk(n_add).indices
                        add_indices = candidate_indices[top_indices]
                        mask_pos[b, gt_idx, add_indices] = 1
