...
class AerialMotionBlur(BaseTransform):
    """Simulate motion blur from UAV flight for aerial image augmentation."""
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        dx = np.cos(direction_rad) * center
        dy = np.sin(direction_rad) * center
        cv2.line(kernel, (center, center), (int(center + dx), int(center + dy)), 1.0, 1)
        if kernel.sum() > 0:
            kernel /= kernel.sum()
        labels["img"] = cv2.filter2D(img, -1, kernel)
        return labels


class AerialShadowIllumination(BaseTransform):
    """Simulate shadow and illumination changes in aerial imagery."""

    def __init__(self, p: float = 0.5, shadow_p: float = 0.3):
        self.p = p
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if random.random() < self.shadow_p:
            ...
            img = np.where(mask[:, :, np.newaxis] == 255, (img * intensity).astype(np.uint8), img)
        illumination = random.uniform(0.7, 1.3)
        img = np.clip(img * illumination, 0, 255).astype(np.uint8)
...
class AerialScaleJitter(BaseTransform):
    """Altitude-aware scale jitter for aerial images, preserving small object visibility."""
...
class AerialPerspectiveTransform(BaseTransform):
    """Simulate perspective transformation from UAV flight altitude and angle variations in aerial imagery."""

    def __init__(self, p: float = 0.5, perspective_factor: float = 0.001):
        self.p = p
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        dx = random.uniform(-self.perspective_factor, self.perspective_factor) * w
        dy = random.uniform(-self.perspective_factor, self.perspective_factor) * h
...
def v8_transforms(dataset, imgsz: int, hyp: IterableSimpleNamespace, stretch: bool = False):
    ...
    aerial_aug = getattr(hyp, "aerial_aug", False)
    ...
    if aerial_aug:
        transforms_list.insert(5, AerialMotionBlur(p=getattr(hyp, "aerial_motion_blur", 0.3)))
        transforms_list.insert(5, AerialShadowIllumination(p=getattr(hyp, "aerial_shadow", 0.5)))
        transforms_list.insert(5, AerialScaleJitter(p=getattr(hyp, "aerial_scale_jitter", 0.5)))
        transforms_list.insert(5, AerialPerspectiveTransform(p=getattr(hyp, "aerial_perspective", 0.5)))
