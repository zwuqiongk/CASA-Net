...
class MFEM(nn.Module):
    """Multi-scale Feature Enhancement Module for CASA-Net.

    Three parallel branches: Primary (feature preservation), Context (multi-scale aggregation from
    adjacent FPN levels), Attention (spatial localization). Feature fusion via channel-wise
    concatenation followed by 1x1 refinement convolution.

    When adjacent FPN features are provided via yaml list-style `from`, the Context branch
    aggregates real cross-scale information. Otherwise, it falls back to multi-dilation
    ...
    def __init__(self, c1, c2=None, adj_ch=None):
        """Initialize MFEM with input/output channels and optional adjacent feature channels.

        Args:
            c1 (int): Input channels.
            c2 (int, optional): Output channels. Defaults to c1.
            adj_ch (list[int], optional): Channel counts of adjacent FPN features.
                Ordered as [adj_up, adj_down, ...] where adj_up is lower-resolution
                and adj_down is higher-resolution. Projection convolutions are created
                automatically to align channels.
        """
        super().__init__()
        c2 = c2 or c1
        adj_ch = adj_ch or []
        ...
        self.primary_conv = Conv(c1, c1, 3, 1)

        self.context_d1 = nn.Conv2d(c1, c1, 3, 1, 1, 1, groups=c1, bias=False)
        self.context_d2 = nn.Conv2d(c1, c1, 3, 1, 2, 2, groups=c1, bias=False)
        self.context_d4 = nn.Conv2d(c1, c1, 3, 1, 4, 4, groups=c1, bias=False)
        self.context_reduce = Conv(c1 * 3, c1, 1, 1)
        self.context_conv = Conv(c1, c1, 3, 1)

        self.adj_projs = nn.ModuleList([Conv(ch, c1, 1, 1) for ch in adj_ch])

        self.spatial_conv1 = nn.Conv2d(2, c1 // 4, 3, 1, 1, bias=False)
        self.spatial_conv2 = nn.Conv2d(c1 // 4, 1, 3, 1, 1, bias=False)

        self.fusion_conv = Conv(c1 * 3, c2, 1, 1)

    def _spatial_attention(self, x):
        """Compute spatial attention map from primary features."""
        avg_out = torch.mean(x, dim=1, keepdim=True)
        max_out, _ = torch.max(x, dim=1, keepdim=True)
        concat = torch.cat([avg_out, max_out], dim=1)
        att = torch.sigmoid(self.spatial_conv2(torch.relu(self.spatial_conv1(concat))))
        return x * att

    def _context_branch_dilated(self, x):
        """Context branch using dilated convolutions (no adjacent features available)."""
        d1 = self.context_d1(x)
        d2 = self.context_d2(x)
        d4 = self.context_d4(x)
        context_raw = torch.cat([d1, d2, d4], dim=1)
        return self.context_conv(self.context_reduce(context_raw))

    def _context_branch_adj(self, x, adj_features):
        """Context branch using adjacent FPN features for true multi-scale context.

        Args:
            x: Current level feature [B, C, H, W].
            adj_features: List of projected adjacent features. Ordered as
                [adj_up, adj_down, ...] where adj_up is lower-resolution and
                adj_down is higher-resolution. All are resized to match x spatially.
        """
        parts = [x]
        for feat in adj_features:
            parts.append(F.interpolate(feat, size=x.shape[2:], mode="nearest"))
        while len(parts) < 3:
            parts.append(torch.zeros_like(x))
        raw = torch.cat(parts[:3], dim=1)
        return self.context_conv(self.context_reduce(raw))

    def forward(self, x):
        """Forward pass supporting both standalone and cross-FPN modes.

        When `from` in yaml is a list (e.g. [[-1, 10, 4], 1, MFEM, [512]]), this
        method receives a list of tensors: [current, adj_up, adj_down, ...].
        Otherwise it receives a single tensor and falls back to dilated convolutions.
        """
        if isinstance(x, list):
            x_main = x[0]
            adj_raw = x[1:]
            if adj_raw and len(self.adj_projs) > 0:
                adj_features = [proj(feat) for feat, proj in zip(adj_raw, self.adj_projs)]
                primary = self.primary_conv(x_main)
                context = self._context_branch_adj(x_main, adj_features)
                attention = self._spatial_attention(primary)
                fused = torch.cat([primary, context, attention], dim=1)
                return self.fusion_conv(fused)
            x = x_main

        primary = self.primary_conv(x)
        context = self._context_branch_dilated(x)
        attention = self._spatial_attention(primary)
        fused = torch.cat([primary, context, attention], dim=1)
        return self.fusion_conv(fused)
